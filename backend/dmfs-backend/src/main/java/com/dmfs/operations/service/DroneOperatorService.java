...
public class DroneOperatorService {
 ...
 @Transactional(readOnly=true) public List<OperationResponse> operations(String email){User u=user(email); return requests.findByStatusIgnoreCaseOrOperatorIdOrderByCreatedAtDesc("PENDING",u.getId()).stream().filter(r -> java.util.Arrays.asList(r.getServiceCatalogue().getRequiredPersonnel().split(",")).contains("DRONE_OPERATOR")).filter(r -> r.getOperator()==null || r.getOperator().getId().equals(u.getId())).map(this::operation).toList();}
 @Transactional public OperationalSurveyResponse accept(Long requestId,String email){User u=user(email); ServiceRequest r=request(requestId); if(!"PENDING".equalsIgnoreCase(r.getStatus())) throw new IllegalArgumentException("This operation is no longer available"); if(r.getOperator()!=null && !r.getOperator().getId().equals(u.getId())) throw new IllegalArgumentException("This operation is assigned to another operator"); if(u.getCompany()==null) throw new IllegalArgumentException("Drone operator has no company assignment"); if(surveys.findByOperatorIdOrderByStartedAtDesc(u.getId()).stream().anyMatch(s -> "SCHEDULED".equalsIgnoreCase(s.getStatus()) || "IN_PROGRESS".equalsIgnoreCase(s.getStatus()))) throw new IllegalArgumentException("You have active work; complete it before accepting another operation"); r.setOperator(u);r.setStatus("ACCEPTED"); Survey s=new Survey();s.setCompany(u.getCompany());s.setServiceRequest(r);s.setOperator(u);s.setSurveyCode("SUR-"+System.currentTimeMillis());s.setSurveyName(r.getServiceCatalogue().getName());s.setDescription(r.getNotes());s.setStatus("SCHEDULED");surveys.save(s); notifyManagement(u,"OPERATION_ACCEPTED", "Drone operator accepted service request #"+r.getId()+"; survey "+s.getSurveyCode()+" is scheduled.");return survey(s);}
